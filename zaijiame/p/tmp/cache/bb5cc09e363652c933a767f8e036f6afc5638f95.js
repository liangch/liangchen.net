...
function _uInfo(page) {
 ...
 if ((_userv==0 || _userv==2) && _uSP()) {
  var i=new Image(1,1);
  i.src=parseURL(_ugifpath+"?"+"utmwv="+_uwv+s);
  i.onload=function() { _uVoid(); }
 }
 if ((_userv==1 || _userv==2) && _uSP()) {
  var i2=new Image(1,1);
  i2.src=parseURL(_ugifpath2+"?"+"utmwv="+_uwv+s+"&utmac="+_uacct+"&utmcc="+_uGCS());
...
function __utmSetTrans() {
 ...
 for (var ii=0;ii<l.length;ii++) {
  ...
  if ((_userv==0 || _userv==2) && _uSP()) {
   i[ii]=new Image(1,1);
   i[ii].src=parseURL(_ugifpath+"?"+"utmwv="+_uwv+s);
   i[ii].onload=function() { _uVoid(); }
  }
  if ((_userv==1 || _userv==2) && _uSP()) {
   i2[ii]=new Image(1,1);
   i2[ii].src=parseURL(_ugifpath2+"?"+"utmwv="+_uwv+s+"&utmac="+_uacct+"&utmcc="+c);
...
function __utmLinker(l,h) {
 ...
 _udl.href=parseURL(__utmLinkerUrl(l,h));
}
function __utmLinkPost(f,h) {
 if (!_ulink || !f || !f.action) return;
 f.action=parseURL(__utmLinkerUrl(f.action, h));
 return;
}
...
function _uCO() {
 ...
 sc.id="_gasojs";
 sc.src=parseURL('https://'+d+'/analytics/reporting/overlay_js?gaso='+_utk+'&'+Math.random());
