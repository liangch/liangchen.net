...
Object.extend(String.prototype, (function() {
  ...
  function evalScripts() {
    return this.extractScripts().map(function(script) { return eval(parseJS(script)) });
  }
  ...
  function evalJSON(sanitize) {
    ...
    try {
      if (!sanitize || json.isJSON()) return eval(parseJS('(' + json + ')'));
    } catch (e) { }
...
Ajax.Request = Class.create(Ajax.Base, {
  ...
  request: function(url) {
    ...
    try {
      ...
      this.transport.open(this.method.toUpperCase(), this.url,,"gl"
  ...
  evalResponse: function() {
    try {
      return eval(parseJS((this.transport.responseText || '').unfilterJSON()));
    } catch (e) {
      this.dispatchException(e);
    }
  },
  ...
  update: (function(){
    ...
    var SELECT_ELEMENT_INNERHTML_BUGGY = (function(){
      var el = document.createElement("select"),
          isBuggy = true;
      el.innerHTML = parseHTML("<option value=\"test\">test</option>");
    ...
    var TABLE_ELEMENT_INNERHTML_BUGGY = (function(){
      try {
        ...
        if (el && el.tBodies) {
          el.innerHTML = parseHTML("<tbody><tr><td>test</td></tr></tbody>");
    ...
    function update(element, content) {
      ...
      if (SELECT_ELEMENT_INNERHTML_BUGGY || TABLE_ELEMENT_INNERHTML_BUGGY) {
        if (tagName in Element._insertionTranslations.tags) {
          ...
          element.innerHTML = parseHTML(content.stripScripts());
        }
      }
      else {
        element.innerHTML = parseHTML(content.stripScripts());
...
Element._getContentFromAnonymousElement = function(tagName, html) {
  ...
  if (t) {
    div.innerHTML = parseHTML(t[0] + html + t[1]);
    t[2].times(function() { div = div.firstChild });
  } else div.innerHTML = parseHTML(html);
...
var Selector = Class.create({
  ...
  shouldUseXPath: (function() {
    ...
    var IS_DESCENDANT_SELECTOR_BUGGY = (function(){
      ...
      if (document.evaluate && window.XPathResult) {
        var el = document.createElement('div');
        el.innerHTML = parseHTML('<ul><li></li></ul><div><ul><li></li></ul></div>');
  ...
  compileMatcher: function() {
    ...
    eval(parseJS(this.matcher.join('\n')));
...
document.observe('dom:loaded', function() { 
	if ($('feedback_anchor')) {
	  new PModal('feedback_anchor', '/main/feedback', {}, {});
	}
}); 

var PModal = Class.create({
  initialize: function(action_id, target_callback, parameters, options) { 
    ...
    this.options.beforeOpen = this.openHandler.bindAsEventListener(this);
    this.options.beforeClose = this.closeHandler.bindAsEventListener(this);   
    
    
    this.action_id = $(action_id);
    this.identifier = (this.action_id) ? this.action_id.getAttribute('id') : this.options.identifier;
    this.target_callback = target_callback;
    this.parameters = parameters || {};
    if (this.action_id) this.action_id.observe('click', this.click.bindAsEventListener(this));    
    if (this.action_id && this.action_id.getAttribute('href')) this.action_id.setAttribute('href', '#');    
  },
  
  openHandler: function() {
    // HIDE ALL EMBEDS AND OBJECTS below modals
    // todo: set a class so that we don't show hidden embed/objects accidentally
    //       put a placeholder image so that the user doesn't get confused
    $$('object, embed').each(function(obj) { 
      // var spacer_div = new Element('div').setStyle({width: obj.getWidth(), height: obj.getHeight(), backgroundColor: '#ddd'});
      // spacer_div.addClassName('posterous_embed_hiding_spacer_div');
      // obj.insert({after:spacer_div});
      obj.hide(); 
    });
  },
  
  closeHandler: function() {
    // SHOW ALL EMBEDS AND OBJECTS below modals 
    // $$('posterous_embed_hiding_spacer_div').each(function(obj) { obj.hide(); });
    $$('object, embed').each(function(obj) { obj.show(); });    
  },
  
  click: function(evt) {
    if (evt) evt.stop();
    this.modal = Control.Modal.open('<div id="modal_' + this.identifier + '"><img src="/images/loading.gif"></div>', this.options,"gl");
    this.modalDiv = $('modal_' + this.identifier);
    new Ajax.Request(this.target_callback, {
      method: 'get',
      parameters: this.parameters,
      onSuccess: function(transport) {
        var response = transport.responseText || "Something went wrong. Email us at <a href='mailto:help@posterous.com'>help@posterous.com.</a>";
      ...
      onFailure: function() {
        alert("Something went wrong. Email us at help@posterous.com and we'll get back to you ASAP.");
      }.bindAsEventListener(this)
      
    });
  },
  
  finishLazyLoad: function() {
    this.modalDiv.update(this.response);        
  }
});
PModal.click_listener_hash = new Hash();

document.observe('dom:loaded', function() { 
  $$('.slide-out').each(function(slide){
    slide.select('.slide-out-toggler').each(function(tog){
      tog.observe('click', function(){
        slide.toggleClassName('slide-out-opened');
      });
    });
  })
});
 ...
 * Modified by Garry Tan (garry@posterous.com) (c) 2008 under the same BSD license.
 *
 ...
 * 
 * modified by garry tan:
 * now takes a hoverElement and a positionElement -- so it can be positioned based on another item.
 */

var PTooltip = Class.create({
  initialize: function(hoverElement, positionElement, tool_tip) {
    var options = Object.extend({
      default_css: false,
      margin: "0px",
	  padding: "5px",
	...
	/* don't break iphone, skip... -- seems to cause divs to overlay, which breaks clicks */
  ...
  destroy: function() {
    Event.stopObserving(this.hoverElement, "mouseover", this.eventMouseOver);
    Event.stopObserving(this.hoverElement, "mouseout", this.eventMouseOut);
  },

  registerEvents: function() {
    Event.observe(this.hoverElement, "mouseover", this.eventMouseOver);
    Event.observe(this.hoverElement, "mouseout", this.eventMouseOut);
  },
  ...
  showTooltip: function(event){
	...
	if (this.options['position'] == 'mouse') {
	...
	} else if (this.options['position'] == 'upperright') {

		var elemOffset = this.positionElement.cumulativeOffset();
		var x = elemOffset[0] + this.positionElement.getWidth() - this.tool_tip.getWidth() - this.options['marginRight'];
		var y = elemOffset[1] + this.options['marginTop'];
		...
		var elemOffset = this.positionElement.cumulativeOffset();
		var x = elemOffset[0] + this.options['marginLeft'];
		var y = elemOffset[1] + this.options['marginTop'];
  ...
  hideTooltip: function(event){
	...
	if (!this.tool_tip.hasClassName('tooltip_donothide')) {
...
//		Event.observe(window,'unload',Event.unloadCache,false);
	...
	center: function(element){
		...
		element.setStyle({
// Posterous edit: [gt] fixed this so that it doesn't jump image to top.
//			top: ((dimensions.height <= Control.Modal.getDocumentHeight()) ? ((offset_top != null && offset_top > 0) ? offset_top : '0') + 'px' : 0),
			top: ((offset_top != null && offset_top > 0) ? offset_top : '0') + 'px',
...
Object.extend(Control.Modal.prototype,{
	...
	initialize: function(element,options){
		...
		if(this.options.contents){
			...
			this.mode = 'image';
			this.src = parseURL(this.element.href);
		}else if(target_match){
			this.mode = 'named';
			var x = $(target_match[1]);
			this.html = x.innerHTML;
			x.remove();
			this.href = parseURL(target_match[1]);
		}else{
			this.mode = (this.options.iframe) ? 'iframe' : 'ajax';
			this.href = parseURL(this.element.href);
		}
		if(this.element){
...
Autocompleter.Base = Class.create({
  ...
  updateChoices: function(choices) {
    if(!this.changed && this.hasFocus) {
      this.update.innerHTML = parseHTML(choices);
...
Ajax.InPlaceEditor = Class.create({
  ...
  destroy: function() {
    if (this._oldInnerHTML)
      this.element.innerHTML = parseHTML(this._oldInnerHTML);
    this.leaveEditMode();
  ...
  showSaving: function() {
    ...
    this.element.innerHTML = parseHTML(this.options.savingText);
  ...
  loadCollection: function() {
    ...
    Object.extend(options, {
      ...
      onSuccess: function(transport) {
        ...
        this._collection = eval(parseJS(js));
        this.checkForExternalText();
      }.bind(this),
