...
function _GoStatsClick (event)
{
    ...
    if (_go_href)
        setTimeout('_go_track_img.src = parseURL(_go_track_src + "&r=' +
                   _GoStatsEscape (_go_href) + '&re=' +
...
function _GoStatsRun ()
{
    ...
    if (_got == 5)
    {
        ...
        _go_img.src = parseURL(_go_img_src);
    ...
    if (_go_fix_png)
    {
        _go_render += '<span id="' + _go_img_id + '" title="' + _gol +
                      '" style="margin:0 4px 4px 0;display:inline-block;border-width:0px;width:' +
    ...
    if (!_go_fix_png)
    {
        ...
        if (_go_img_element)
            _go_img_element.src = parseURL(_go_img_src);
    }
}
